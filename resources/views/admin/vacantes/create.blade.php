      ...
      <form action="{{ route('admin.vacante.store') }}" method="POST" noValidate class="space-y-3">
        ...
        <div>
          <label for="requisitos"
            class="font-medium mb-2 after:ml-0.5 after:text-red-500 after:content-['*']">Requisitos del puesto</label>
          <input id="requisitos" name="requisitos" value="{{ old('requisitos') }}" type="text"
            placeholder="Requisitos del puesto" class="disabled:bg-nav-900 disabled:border-nav-900 border-link-100 focus:shadow-link-200 w-full rounded-md border-2 bg-transparent p-2 outline-none focus:shadow-md placeholder:text-slate-300 dark:placeholder:text-slate-400 mt-2 @error('requisitos')
          dark:border-alerts-500
          @enderror">
          @error('requisitos')
