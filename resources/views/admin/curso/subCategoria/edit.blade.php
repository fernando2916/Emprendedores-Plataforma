...
<x-breadcrumb :items="[
    ...
    ['url' => '', 'label' => 'Editar Subcategoria']  {{-- Último elemento desactivado --}}
]" />

<div class="bg-light-200 dark:bg-cont-100 p-5 rounded-lg">
  <div class="">
    <div class="">
      <h3 class="text-3xl font-bold">Editar Sub Categoria</h3>
    </div>
    <div>
      <form action="{{ route('admin.subcategorias.update', $subcategoria) }}" method="POST" noValidate>
        @csrf
        @method('PUT')
        <div>
          ...
          <input id="name" name="nombre_subcategoria" value="{{ old('nombre_subcategoria', $subcategoria->nombre_subcategoria) }}" type="text"
        ...
        <div>
          ...
          <select name="categoria_cursos_id" id="categoria_cursos_id"
           ...
           <option value="{{ $category->id }}"
            @if ($subcategoria->categoria_cursos_id == $category->id) selected @endif>
